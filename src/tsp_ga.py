...
class Gene:
    ...
    def get_distance_to(self, dest):
        ...
        distance = math.sqrt(pow(dest[0] - origin[0], 2) + pow(dest[1] - origin[1], 2))
        Gene.__distances_table[forward_key] = distance
...
class Population:  # Population of individuals
    ...
    def get_fittest(self):
        fit = self.individuals[0]
        for route in self.individuals:
            if route.fitness > fit.fitness:
                fit = route
